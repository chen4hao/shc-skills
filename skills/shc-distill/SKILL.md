...
- **子代理完成後**，主代理從各子代理的回傳結果中取得完整筆記內容，用 **Write 工具統一寫入**所有分段筆記檔案（可並行發出多個 Write 呼叫）
...
- **主代理收到結果後**，統一用 Write 工具將翻譯批次檔寫入專案輸出目錄
- 翻譯前，先將 `/tmp/distill-{VIDEO_ID}/` 中清理後的英文 SRT 複製到專案輸出目錄
- 翻譯完成、主代理寫入所有批次檔後，合併腳本從專案輸出目錄讀取批次檔
...
# ZH→EN 流程（中文原文）：
uv run python3 $SCRIPTS/split_batches.py "/tmp/distill-{VIDEO_ID}/{中文clean檔名}" "{專案輸出目錄}" "{VIDEO_ID}" 5 zh
```
   產出檔案命名為 `{VIDEO_ID}_{LANG}_batch_1.srt`、`{VIDEO_ID}_{LANG}_batch_2.srt` 等（LANG 為 `en` 或 `zh`）。
2. 用 **Agent 工具**一次性啟動所有批次的子代理（在同一個訊息中發出多個 Agent 呼叫），每個子代理：
   - `model: "sonnet"` — 使用 sonnet 模型節省成本
   - `mode: "dontAsk"` — 用於 Read 等工具的權限
   - `run_in_background: true` — 背景執行，並行處理
   - 任務：從**專案輸出目錄**讀取 `{VIDEO_ID}_{SOURCE_LANG}_batch_{X}.srt` **整個檔案**，翻譯為目標語言，**將完整翻譯結果作為回傳內容輸出**（不要嘗試寫入檔案）
   - EN→ZH 流程：讀取 `{VIDEO_ID}_en_batch_{X}.srt`，翻譯為繁體中文
   - ZH→EN 流程：讀取 `{VIDEO_ID}_zh_batch_{X}.srt`，翻譯為英文

**子代理 prompt 範本**（根據翻譯方向調整語言對）：
```
...
5. **主代理寫入翻譯批次檔**：從各子代理的回傳結果中取得翻譯後的 SRT 內容，用 **Write 工具**寫入 `{專案輸出目錄}/{VIDEO_ID}_{TARGET_LANG}_batch_{X}.srt`（可並行發出多個 Write 呼叫）
