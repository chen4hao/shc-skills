...
2. 用 **Agent 工具**一次性啟動所有批次的子代理（在同一個訊息中發出多個 Agent 呼叫），每個子代理：
   ...
   - `run_in_background: true` — 背景執行，並行處理
   - 任務：從**專案輸出目錄**讀取 `{VIDEO_ID}_en_batch_{X}.srt` **整個檔案**，翻譯為繁體中文，**將完整翻譯結果作為回傳內容輸出**（不要嘗試寫入檔案）
...
5. **主代理寫入翻譯批次檔**：從各子代理的回傳結果中取得翻譯後的 SRT 內容，用 **Write 工具**寫入 `{專案輸出目錄}/{VIDEO_ID}_zh_batch_{X}.srt`（可並行發出多個 Write 呼叫）
